...
public class DatSanChiTietActivity extends AppCompatActivity {
    ...
    TextView tv_tensan, tvCustomer, tvMocTg,
            tvDate, tvSanBongMoney, tvAllMoney, tvMoneyCustomer, tvShowCaThiDau, tvChiPhi;
    ...
    public void datSan() {
        if (count <= 0) {
            Toast.makeText(this, "You need select at least 1 schedule", Toast.LENGTH_SHORT).show();
        } else {
            if (!isUpdate) {
                if (customer == null) {
                    Toast.makeText(this, "Please choose customer", Toast.LENGTH_SHORT).show();
                } else {
                    ...
                    if (customer.getCoin() < total) {
                        ...
                        for (int i = 0; i < typeSelect.length; i++) {
                            if (typeSelect[i] == type_cancel) {
                                TimeOrderDetails timeOrderDetails = TimeOrderDetails.getInstance();
                ...
                if (customer.getCoin() < total) {
                    ...
                    for (int i = 0; i < typeSelect.length; i++) {
                        if (typeSelect[i] == type_cancel && types[i] == 0) {
                            TimeOrderDetails timeOrderDetails1 = TimeOrderDetails.getInstance();
...
//        btnServiceDetails = findViewById(R.id.btn_chitiet_datsanchitiet);
        ...
        for (int i = 0; i < listSelect.size(); i++) {
            ...
            listSelect.get(i).setOnClickListener(v -> {
                if (typeSelect[finalI] == type_add && count < maxCount) {
                    ...
                    Toast.makeText(this, "Can't cancelled", Toast.LENGTH_SHORT).show();
                } else if (typeSelect[finalI] == type_addGray) {
                    Toast.makeText(this, "Overtime", Toast.LENGTH_SHORT).show();
                } else if (typeSelect[finalI] == type_full) {
                    Toast.makeText(this, "Have been booked", Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
    ...
    public void setOnCLickForView() {
        tvDate.setOnClickListener(v -> {
            if (!isUpdate) {
                if(count>0){
                    Toast.makeText(DatSanChiTietActivity.this, "You need to cancel day of:  " + datePlay, Toast.LENGTH_SHORT).show();
                }else {
        ...
        tvCustomer.setOnClickListener(v -> {
            if (!isUpdate) {
                ...
                Toast.makeText(this, "You can't select a customer when updating the information sheet", Toast.LENGTH_SHORT).show();
            }
        });
