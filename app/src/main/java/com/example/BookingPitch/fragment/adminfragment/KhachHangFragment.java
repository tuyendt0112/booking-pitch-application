...
import com.example.BookingPitch.activity.user.NapTienActivity;
import com.example.BookingPitch.adapter.admin.CustomerAdapter;
import com.example.BookingPitch.database.MyDatabase;
import com.example.BookingPitch.facade.CheckCustomerAddingFacade;
import com.example.BookingPitch.facade.CheckCustomerUpdateFacade;
...
public class KhachHangFragment extends Fragment {
    ...
    public void createDialogAdd(){
        ...
        btn.setOnClickListener(v->{
            ...
            CheckCustomerAddingFacade CustomerAddingFacade = new CheckCustomerAddingFacade(phone,name,address,pass,pass2);
            if (!CustomerAddingFacade.AddingCustomerFacade())
            {
                ...
                if(MyDatabase.getInstance(getContext()).customerDAO().getCustomerWithPhone(phone,-1).size()==0) {
                    ...
                    Toast.makeText(getContext(), "Adding customers successful", Toast.LENGTH_SHORT).show();
                    customerList = MyDatabase.getInstance(getContext()).customerDAO().getAll();
                    adapter.setData(customerList);
    ...
    public void createDialogUpdate(Customer c){
        ...
        btn.setOnClickListener(v->{
            ...
            CheckCustomerUpdateFacade CustomerUpdateFacade = new CheckCustomerUpdateFacade(phone,name,address);
            if (!CustomerUpdateFacade.UpdateCustomerFacade())
            {
                tvCheckPhone.setText("* Invalid information");
                ...
                if(MyDatabase.getInstance(getContext()).customerDAO().getCustomerWithPhone(phone,c.getId()).size()==0) {
                    ...
                    Toast.makeText(getContext(), "Update customer successfully", Toast.LENGTH_SHORT).show();
                    customerList = MyDatabase.getInstance(getContext()).customerDAO().getAll();
                    adapter.setData(customerList);
