...
import com.example.BookingPitch.activity.admin.CategoryPitchActivity;
import com.example.BookingPitch.adapter.admin.PitchAdapter;
...
public class SanBongFragment extends Fragment {
    ...
    public void createDialogAdd() {
        ...
        btnThem.setOnClickListener(v -> {
            ...
            if (name.equals("")) {
                    tvCheck.setText("The pitch name cannot be left blank");
                tvCheck.setVisibility(View.VISIBLE);
            } else {
                if (MyDatabase.getInstance(getContext()).pitchDao().getPitchWithName(name).size() > 0) {
                    tvCheck.setText("The field name already exists");
                    tvCheck.setVisibility(View.VISIBLE);
                } else {
                    tvCheck.setVisibility(View.INVISIBLE);
                    Pitch pitch = Pitch.getInstance();
                    pitch.setName(name);
                    ...
                    Toast.makeText(getContext(), "Adding football field successful", Toast.LENGTH_SHORT).show();
    ...
    public void createDialogUpdate(Pitch p) {
        ...
        tvTitle.setText("CUpdate football field");
        TextView tvCheck = dialog.findViewById(R.id.tv_check_name_dialog_add_pitch);
        ...
        btnSua.setOnClickListener(v -> {
            String name = edtName.getText().toString();
            if (name.equals("")) {
                tvCheck.setText("The field name cannot be left blank");
                tvCheck.setVisibility(View.VISIBLE);
